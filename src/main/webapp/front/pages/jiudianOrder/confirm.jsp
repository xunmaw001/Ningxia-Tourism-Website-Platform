            ...
            <table class="layui-table" lay-skin="nob" style="border: 3px dotted #EEEEEE;margin: 20px 0;">
                ...
                <tr v-for="(item,index) in dataList" v-bind:key="index">
                    <td style="display: flex;text-align: left;">
                        <img :src="baseUrl+item.jiudianPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                        <div style="margin-left: 10px;margin-top: 40px;">
                            {{item.jiudianName}}
                        </div>
                    </td>
                    <td  style="width: 150px;">{{item.jiudianNewMoney}} RMB</td>
                    <td style="width: 140px;">
                        {{item.buyNumber}}
                    </td>
                    <td style="width: 100px;">{{(item.jiudianNewMoney*item.buyNumber).toFixed(2)}} RMB</td>
                </tr>
                </tbody>
            </table>

            <div class="btn-container">
                <div v-if="this.jiudianOrderPaymentTypes == 1" style="margin-right: 150px">
				...
				<div v-if="this.jiudianOrderPaymentTypes == 2" style="margin-right: 150px;margin-top: 20px">
		...
		<script>
            var vue = new Vue({
                ...
                data: {
                    ...
                    baseUrl:"",
                    dataList: [],
                    jiudianOrderTime:new Date(),
                    jiudianOrderPaymentTypes: 1,
                    zhekou:1,
                    // 当前用户信息
                    user: {}
                },
                computed: {
                    totalMoney: function() {
                        ...
                        for (var item of this.dataList) {
                            total += item.jiudianNewMoney * item.buyNumber
                        }
                        return total;
                    }
                },
                methods: {
                    ...
                    ,payClick() {
                    let data={
                        buyNumber:JSON.parse(localStorage.getItem('jiudians'))[0].buyNumber,
                        jiudianId:JSON.parse(localStorage.getItem('jiudians'))[0].jiudianId,
                        jiudians:localStorage.getItem('jiudians'),
			            yonghuId: localStorage.getItem('userid'),
                            jiudianOrderTime:getDatetimeFormat(vue.jiudianOrderTime),//年月日时分秒
                    }
                        // 获取商品详情信息
                        layui.http.requestJson(`jiudianOrder/add`, 'POST', data, (res) => {
                            if(res.code == 0){
                                layui.layer.msg('下单成功', {
                                    time: 2000,
                                    icon: 6
                                },function (){
                                    window.location.href='../jiudianOrder/list.jsp';
                                });
                            }else{
            ...
            layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取酒店推荐购买的清单列表
				var jiudians = localStorage.getItem('jiudians');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(jiudians);
