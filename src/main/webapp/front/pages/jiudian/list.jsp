    ...
    <div class="portfolio-agile" style="width: 100%">
        <div class="main">
            <div v-for="(item,index) in dataList" v-bind:key="index" class="view view-seventh list-item3B" style="height: 350px;">
                <a @click="jumpCheck('../jiudian/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="b-link-stripe b-animate-go thickbox">
                    <img :src="item.jiudianPhoto?baseUrl+item.jiudianPhoto.split(',')[0]:''"  style="height: 350px;width:100%;" />
                    <div class="mask">
                        <h4>
                            <b>{{item.jiudianName}}</b>
                            <br>
                            <b v-if="item.jiudianNewMoney">{{item.jiudianNewMoney}} RMB</b>
                            <b v-else>{{item.jiudianValue}}</b>
                        </h4>
                    </div>
                </a>
            </div>
            ...
            <div class="col-lg-4 event-right wthree-event-right">
                ...
                <input type="text" v-model="searchForm.jiudianName" placeholder="酒店名称" autocomplete="off"
                       class="search-input">                         
                                                <div style="display: flex;justify-content: space-evenly;align-items: center;">
                        <div id="btn-search" class="button-submit"><i class="layui-icon layui-icon-search"></i>搜索</div>
                        <div v-if="isAuth('jiudian','新增')"  @click="jump('../jiudian/add.jsp')" class="button-submit"><i class="layui-icon">&#xe654;</i>添加</div>
                    </div>
                </form>
                <div class="tags tags1">
                    ...
                    <ul>
                        <li class="thisTableType-search">
                            <a :class='searchForm.jiudianTypes=="" || searchForm.jiudianTypes==null?"thisTableType-search-hover":""'>全部</a>
                        </li>
                        <li v-for="(item,index) in jiudianTypesList" :key="item.codeIndex" :index="item.codeIndex" class="thisTableType-search" >
                            <a :class='searchForm.jiudianTypes==item.codeIndex?"thisTableType-search-hover":""'>{{item.indexName}}</a>
                        </li>
                    </ul>
                </div>
            </div>
