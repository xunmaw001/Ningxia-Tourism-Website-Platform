...
<body style="background: #fff; ">
    ...
    <!-- 当前表的 -->
                ...
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    ...
                    <div class="layui-input-block">
                        <select name="jingdainCollectionTypes" id="jingdainCollectionTypes" lay-filter="jingdainCollectionTypes" :disabled="ro.jingdainCollectionTypes">
                            <option v-for="(item,index) in jingdainCollectionTypesList" v-bind:key="index" :value="item.codeIndex" :selected="detail.jingdainCollectionTypes == item.codeIndex?true:false" :key="item.codeIndex">{{ item.indexName }}</option>
                        </select>
                    </div>
                </div>
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
    ...
    <script>
        ...
        layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
           ...
           // 上传文件
            ...
            //修改
            if(http.getParam('jingdainCollectionId')){
                http.request(`jingdainCollection/info/${http.getParam('jingdainCollectionId')}`, 'get', {}, function(data) {
            ...
            form.on('submit(thisSubmit)', function (data) {
                ...
                if(http.getParam('jingdainCollectionId')){
                    data["id"]= http.getParam('jingdainCollectionId')
                }
                // 提交数据
                http.requestJson(`jingdainCollection/${http.getParam("jingdainCollectionId")?"update":"add"}`, 'post', data, function (res) {
                    if(res.code == 0){
                        layer.msg('提交成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            vue.jump("../jingdainCollection/list.jsp");
                                http.remove('jingdainCollectionId')
                            // back();
                        });
                    }else{
       ...
       // 类型的查询
       function jingdainCollectionTypesSelect() {
           //填充下拉框选项
           layui.http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jingdain_collection_types", "GET", {}, (res) => {
               if(res.code == 0){
                   vue.jingdainCollectionTypesList = res.data.list;
               }
           });
       }
