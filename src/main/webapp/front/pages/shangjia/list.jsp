    ...
    <div class="portfolio-agile" style="width: 100%">
        <div class="main">
            <div v-for="(item,index) in dataList" v-bind:key="index" class="view view-seventh list-item3B" style="height: 350px;">
                <a @click="jumpCheck('../shangjia/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="b-link-stripe b-animate-go thickbox">
                    <img :src="item.shangjiaPhoto?baseUrl+item.shangjiaPhoto.split(',')[0]:''"  style="height: 350px;width:100%;" />
                    <div class="mask">
                        <h4>
                            <b>{{item.shangjiaName}}</b>
                            <br>
                            <b v-if="item.shangjiaNewMoney">{{item.shangjiaNewMoney}} RMB</b>
                            <b v-else>{{item.shangjiaValue}}</b>
                        </h4>
                    </div>
                </a>
            </div>
            ...
            <div class="col-lg-4 event-right wthree-event-right">
                ...
                <input type="text" v-model="searchForm.shangjiaName" placeholder="商家名称" autocomplete="off"
                       class="search-input">                                                    <div style="display: flex;justify-content: space-evenly;align-items: center;">
                        ...
                        <div v-if="isAuth('shangjia','新增')"  @click="jump('../shangjia/add.jsp')" class="button-submit"><i class="layui-icon">&#xe654;</i>添加</div>
                    </div>
                </form>
            </div>
        </div>
