...
<!-- 标题 -->
     <div class="index-title sub_backgroundColor" :style='{"padding":"10px","boxShadow":"0 0 2px rgba(160, 67, 26, 1)","margin":"10px auto","borderColor":"rgba(0,0,0,.3)","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","borderWidth":"0","fontSize":"20px","borderStyle":"solid","height":"auto"}'>
         <span>USER / COLLECTION</span><span>商品收藏收藏</span>
     </div>
    <!-- 标题 -->
        ...
        <!-- 个人中心菜单 config.js-->
            <div class="left-container" :style='{"backgroundColor":"#fff","padding":"0","boxShadow":"0px 0px 1px rgba(181, 181, 181, 1)","margin":"0","borderColor":"rgba(191, 191, 191, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"0","borderWidth":"0 0 1px 0","width":"20%","borderStyle":"solid"}'>
                <ul class="layui-nav layui-nav-tree">
                    <li v-for="(item,index) in centerMenu" v-bind:key="index" class="layui-nav-item" :class="item.url=='../shangpinCollection/list.jsp'?'layui-this':''">
        ...
        <div class="right-container sub_borderColor" :style='{"padding":"20px","boxShadow":"0px rgba(255,0,0,.8)","margin":"0","backgroundColor":"#fff","borderRadius":"0","borderWidth":"1px","borderStyle":"solid","width":"80%"}'>
            <div style="display: flex;flex-wrap: wrap;justify-content: space-evenly;">
                <div v-for="(item,index) in dataList" :key="index" style="flex: 0 0 25%;padding: 3px">
                    ...
                    <!-- class="animation-box"-->
                        <img @click="jumpCheck('../shangpin/detail.jsp?id='+item.shangpinId , item.shangpinDelete == null?'':item.shangpinDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)"
                             ...
                             :src="item.shangpinPhoto?baseUrl+item.shangpinPhoto.split(',')[0]:''">
                        <div @click="jumpCheck('../shangpin/detail.jsp?id='+item.shangpinId , item.shangpinDelete == null?'':item.shangpinDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                            <div v-if="item.shangpinNewMoney"
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`shangpinCollection/update`, 'post', {
                    id:id,
//                    shangpinCollectionTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`shangpinCollection/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/shangpinCollection/list2.jsp")

        // var id = http.getParam('id');

        //类型的动态搜素
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.shangpinCollectionTypes = $(this).attr('index');
            pageList();
        });
           ...
           function shangpinCollectionTypesSelect() {
            ...
            function pageList() {
                // 获取列表数据
                http.request('shangpinCollection/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('shangpinCollection/list', 'get', vue.searchForm, function (res1) {
