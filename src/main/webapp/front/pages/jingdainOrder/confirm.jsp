            ...
            <table class="layui-table" lay-skin="nob" style="border: 3px dotted #EEEEEE;margin: 20px 0;">
                ...
                <tr v-for="(item,index) in dataList" v-bind:key="index">
                    <td style="display: flex;text-align: left;">
                        <img :src="baseUrl+item.jingdainPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                        <div style="margin-left: 10px;margin-top: 40px;">
                            {{item.jingdainName}}
                        </div>
                    </td>
                    <td  style="width: 150px;">{{item.jingdainNewMoney}} RMB</td>
                    <td style="width: 140px;">
                        {{item.buyNumber}}
                    </td>
                    <td style="width: 100px;">{{(item.jingdainNewMoney*item.buyNumber).toFixed(2)}} RMB</td>
                </tr>
                </tbody>
            </table>

            <div class="btn-container">
                <div v-if="this.jingdainOrderPaymentTypes == 1" style="margin-right: 150px">
				...
				<div v-if="this.jingdainOrderPaymentTypes == 2" style="margin-right: 150px;margin-top: 20px">
		...
		<script>
            var vue = new Vue({
                ...
                data: {
                    ...
                    baseUrl:"",
                    dataList: [],
                    jingdainOrderTime:new Date(),
                    jingdainOrderPaymentTypes: 1,
                    zhekou:1,
                    // 当前用户信息
                    user: {}
                },
                computed: {
                    totalMoney: function() {
                        ...
                        for (var item of this.dataList) {
                            total += item.jingdainNewMoney * item.buyNumber
                        }
                        return total;
                    }
                },
                methods: {
                    ...
                    ,payClick() {
                    let data={
                        buyNumber:JSON.parse(localStorage.getItem('jingdains'))[0].buyNumber,
                        jingdainId:JSON.parse(localStorage.getItem('jingdains'))[0].jingdainId,
                        jingdains:localStorage.getItem('jingdains'),
			            yonghuId: localStorage.getItem('userid'),
                            jingdainOrderTime:getDatetimeFormat(vue.jingdainOrderTime),//年月日时分秒
                    }
                        // 获取商品详情信息
                        layui.http.requestJson(`jingdainOrder/add`, 'POST', data, (res) => {
                            if(res.code == 0){
                                layui.layer.msg('下单成功', {
                                    time: 2000,
                                    icon: 6
                                },function (){
                                    window.location.href='../jingdainOrder/list.jsp';
                                });
                            }else{
            ...
            layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload'], function() {
                ...
                // 获取景点介绍购买的清单列表
				var jingdains = localStorage.getItem('jingdains');
				// 转换成json类型，localstorage保存的是string数据
				vue.dataList = JSON.parse(jingdains);
