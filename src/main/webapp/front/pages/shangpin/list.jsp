    ...
    <div class="portfolio-agile" style="width: 100%">
        <div class="main">
            <div v-for="(item,index) in dataList" v-bind:key="index" class="view view-seventh list-item3B" style="height: 350px;">
                <a @click="jumpCheck('../shangpin/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="b-link-stripe b-animate-go thickbox">
                    <img :src="item.shangpinPhoto?baseUrl+item.shangpinPhoto.split(',')[0]:''"  style="height: 350px;width:100%;" />
                    <div class="mask">
                        <h4>
                            <b>{{item.shangpinName}}</b>
                            <br>
                            <b v-if="item.shangpinNewMoney">{{item.shangpinNewMoney}} RMB</b>
                            <b v-else>{{item.shangpinValue}}</b>
                        </h4>
                    </div>
                </a>
            </div>
            ...
            <div class="col-lg-4 event-right wthree-event-right">
                ...
                <input type="text" v-model="searchForm.shangpinName" placeholder="商品名称" autocomplete="off"
                       class="search-input">                         
                                                <div style="display: flex;justify-content: space-evenly;align-items: center;">
                        <div id="btn-search" class="button-submit"><i class="layui-icon layui-icon-search"></i>搜索</div>
                        <div v-if="isAuth('shangpin','新增')"  @click="jump('../shangpin/add.jsp')" class="button-submit"><i class="layui-icon">&#xe654;</i>添加</div>
                    </div>
                </form>
                <div class="tags tags1">
                    ...
                    <ul>
                        <li class="thisTableType-search">
                            <a :class='searchForm.shangpinTypes=="" || searchForm.shangpinTypes==null?"thisTableType-search-hover":""'>全部</a>
                        </li>
                        <li v-for="(item,index) in shangpinTypesList" :key="item.codeIndex" :index="item.codeIndex" class="thisTableType-search" >
                            <a :class='searchForm.shangpinTypes==item.codeIndex?"thisTableType-search-hover":""'>{{item.indexName}}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
//                    shangpinTypes:1,
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/shangpin/list.jsp")
        ...
        vue.searchForm.shangjiaId = http.getParam('shangjiaId');
