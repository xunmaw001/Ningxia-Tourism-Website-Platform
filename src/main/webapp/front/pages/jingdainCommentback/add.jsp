...
<body style="background: #fff; ">
    ...
    <!-- 当前表的 -->
            ...
            <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                ...
                <div class="layui-input-block">
                    <textarea class="main_borderColor layui-input" style="width: 100%;height: 100px;border-radius: 5px;padding: 10px" v-model="detail.jingdainCommentbackText" lay-verify="required" :readonly="ro.jingdainCommentbackText" name="jingdainCommentbackText" id="jingdainCommentbackText" autocomplete="off"></textarea>
                </div>
            </div>
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                <label  :style='{"width":"auto","borderColor": "rgba(255, 255, 255, 0)","padding":"0 12px 0 0","backgroundColor":"rgba(255, 255, 255, 0)","fontSize":"15px","color":"#333","textAlign":"left"}' class="layui-form-label">
    ...
    <script>
        ...
        layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
            ...
            localStorage.setItem("goUtl","./pages/jingdainCommentback/list.jsp")
           ...
           // 上传文件
            ...
            //修改
            if(http.getParam('jingdainCommentbackId')){
                http.request(`jingdainCommentback/info/${http.getParam('jingdainCommentbackId')}`, 'get', {}, function(data) {
                    vue.detail = data.data
                    jquery("#insertTime").val(vue.detail.insertTime)
                    jquery("#updateTime").val(vue.detail.updateTime)
            ...
            form.on('submit(thisSubmit)', function (data) {
                data = data.field;
                if(http.getParam('jingdainCommentbackId')){
                    data["id"]= http.getParam('jingdainCommentbackId')
                }
                // 提交数据
                http.requestJson(`jingdainCommentback/${http.getParam("jingdainCommentbackId")?"update":"add"}`, 'post', data, function (res) {
                    if(res.code == 0){
                        layer.msg('提交成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            vue.jump("../jingdainCommentback/list.jsp");
                                http.remove('jingdainCommentbackId')
                            // back();
                        });
                    }else{
