...
<div style="width: 1000px;margin: 10px auto;height: auto;">
        ...
        <div style="width: 50%;height: auto;">
            <div class="particulars" style="text-align: center;font-size: 18px;">
                <span>{{title}}</span>
                  ...
                  <div v-if="detail.jiudianCollectionTypes" class="particulars">
                      <span class="displayBianhao">类型:</span>
                      <span class="dataExhibition"> {{detail.jiudianCollectionValue}} </span>
                  </div>
        </div>
        <!-- 图片 -->
        <div style="width: 50%;height: auto;">
    ...
    <div style="width: 100%;">
        <div  style="display: flex;margin-top: 30px;justify-content: center;border-bottom:1px dotted var(--publicMainColor) ;border-top:1px dotted var(--publicMainColor) ;padding: 20px;align-items: center;">
            <!--
             <button @click="jiudianCollectionYuyue()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                 立即预约
             </button>
        -->
        </div>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            data: {
                ...
                //系统推荐
                jiudianCollectionRecommendList: [],
                    // 当前详情页表
            ...
            methods: {
                //分享
                share(){
                    let element = createElement("http://localhost:8080/ningxialvyouwangzhan/front/index.jsp?url="+"./pages/jiudianCollection/detail.jsp?id="+this.id);
                    element.select();
                ...
                //预约
                jiudianCollectionYuyue(){
                    let _this = this;
                    var mymessage = confirm("确定要预约吗？");if(!mymessage){return false;}
                    // localStorage.setItem('jiudianCollectionId', _this.detail.id);
                    // _this.jump("../jiudianCollectionOrder/add.jsp");

                    let data={
                        jiudianCollectionId:_this.detail.id,
                        yonghuId:localStorage.getItem("userid"),
                        jiudianCollectionOrderYesnoTypes:1,
                    }
                    // 提交数据
                    layui.http.requestJson('jiudianCollectionOrder/add', 'post', data, function (res) {
                        if(res.code == 0) {
                            layui.layer.msg('预约成功', {
                                time: 2000,
                                icon: 6
                            }, function () {
                                _this.jump("../jiudianCollectionOrder/list.jsp");
                            });
                        }else{
        ...
        layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'util'], function () {
            ...
            localStorage.setItem("goUtl","./pages/jiudianCollection/detail.jsp?id="+ http.getParam('id'))
            ...
            http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
                // 详情信息
                vue.detail = res.data;
                vue.title = vue.detail.jiudianCollectionName;
                // 轮播图片
            ...
            http.request(`jiudianCollection/list`, 'get', {
                page: 1,
                limit: 5,
                jiudianCollectionTypes: vue.detail.jiudianCollectionTypes,
            }, function (res) {
                vue.jiudianCollectionRecommendList = res.data.list;
            });
