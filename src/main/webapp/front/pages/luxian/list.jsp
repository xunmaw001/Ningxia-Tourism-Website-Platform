    ...
    <div class="portfolio-agile" style="width: 100%">
        <div class="main">
            <div v-for="(item,index) in dataList" v-bind:key="index" class="view view-seventh list-item3B" style="height: 350px;">
                <a @click="jumpCheck('../luxian/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="b-link-stripe b-animate-go thickbox">
                    <img :src="item.luxianPhoto?baseUrl+item.luxianPhoto.split(',')[0]:''"  style="height: 350px;width:100%;" />
                    <div class="mask">
                        <h4>
                            <b>{{item.luxianName}}</b>
                            <br>
                            <b v-if="item.luxianNewMoney">{{item.luxianNewMoney}} RMB</b>
                            <b v-else>{{item.luxianValue}}</b>
                        </h4>
                    </div>
                </a>
            </div>
            ...
            <div class="col-lg-4 event-right wthree-event-right">
                ...
                <input type="text" v-model="searchForm.luxianName" placeholder="路线名称" autocomplete="off"
                       class="search-input">                                 
                                        <div style="display: flex;justify-content: space-evenly;align-items: center;">
                        <div id="btn-search" class="button-submit"><i class="layui-icon layui-icon-search"></i>搜索</div>
                        <div v-if="isAuth('luxian','新增')"  @click="jump('../luxian/add.jsp')" class="button-submit"><i class="layui-icon">&#xe654;</i>添加</div>
                    </div>
                </form>
                <div class="tags tags1">
                    ...
                    <ul>
                        <li class="thisTableType-search">
                            <a :class='searchForm.luxianTypes=="" || searchForm.luxianTypes==null?"thisTableType-search-hover":""'>全部</a>
                        </li>
                        <li v-for="(item,index) in luxianTypesList" :key="item.codeIndex" :index="item.codeIndex" class="thisTableType-search" >
                            <a :class='searchForm.luxianTypes==item.codeIndex?"thisTableType-search-hover":""'>{{item.indexName}}</a>
                        </li>
                    </ul>
                </div>
            </div>
