...
<body style="background: #fff; ">
            ...
            <form class="layui-form layui-form-pane add" lay-filter="myForm">
                ...
                <!--<div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    ...
                    <div class="layui-input-block">
                        <select name="luxianId" id="luxianId" lay-filter="luxianId">
                            <option v-for="(item,index) in luxianList" v-bind:key="index" :value="item.id" :selected="detail.luxianId == item.id?true:false" :key="item.id">{{ item.luxianName }}</option>
                        </select>
                    </div>
                </div>-->
                <!-- 级联表 用户 -->
    ...
    <!-- 当前表的 -->
                <!-- 级联表的表id -->
                <input type="hidden" :value="detail.luxianId" id="luxianId" name="luxianId"/>
                <!-- 级联表的表id -->
                ...
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    ...
                    <div class="layui-input-block">
                        <select name="luxianCollectionTypes" id="luxianCollectionTypes" lay-filter="luxianCollectionTypes" :disabled="ro.luxianCollectionTypes">
                            <option v-for="(item,index) in luxianCollectionTypesList" v-bind:key="index" :value="item.codeIndex" :selected="detail.luxianCollectionTypes == item.codeIndex?true:false" :key="item.codeIndex">{{ item.indexName }}</option>
                        </select>
                    </div>
                </div>
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            data: {
                ...
                detail: {
                    luxianId: '',
                    yonghuId: '',
                    luxianCollectionTypes: '',//数字
                    luxianCollectionValue: '',//数字对应的值
                    insertTime: '',
                    createTime: '',
                },
                luxianList:[],//级联表列表 路线
        ...
        layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
            ...
            localStorage.setItem("goUtl","./pages/luxianCollection/list.jsp")

            //var luxianId = http.getParam('luxianId');
            let luxianId = localStorage.getItem("luxianId")
            if(luxianId != null && luxianId != ""){
                //单个 路线查询
                initializationLuxian(luxianId);
            }
            //列表 路线查询
            initializationLuxianList();
           ...
           // 上传文件
            ...
            //修改
            if(http.getParam('luxianCollectionId')){
                http.request(`luxianCollection/info/${http.getParam('luxianCollectionId')}`, 'get', {}, function(data) {
            ...
            form.on('submit(thisSubmit)', function (data) {
                ...
                if(http.getParam('luxianCollectionId')){
                    data["id"]= http.getParam('luxianCollectionId')
                }
                // 提交数据
                http.requestJson(`luxianCollection/${http.getParam("luxianCollectionId")?"update":"add"}`, 'post', data, function (res) {
                    if(res.code == 0){
                        layer.msg('提交成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            vue.jump("../luxianCollection/list.jsp");
                                http.remove('luxianCollectionId')
                            // back();
                        });
                    }else{
       ...
       function luxianCollectionTypesSelect() {
           //填充下拉框选项
           layui.http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=luxian_collection_types", "GET", {}, (res) => {
               if(res.code == 0){
                   vue.luxianCollectionTypesList = res.data.list;
               }
           });
       }
        ...
        //单个 路线查询
        function initializationLuxian(id){
            layui.http.request("luxian/detail/" + id, 'get', {}, function (res) {
                if(res.code ==0){
                    vue.luxian = res.data;
                    vue.detail.luxianId=vue.luxian.id//赋值给detail
                }
            });
        }
        //列表 路线查询
        function initializationLuxianList(){
            layui.http.request("luxian/page?page=1&limit=100&sort=&order=", 'get', {}, function (res) {
                if(res.code ==0){
