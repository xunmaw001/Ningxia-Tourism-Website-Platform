...
<body>
<div class="modal fade" id="shangpinCommentbackModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true">
                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <div class="table-responsive mb-3" id="tableDiv">
                            <div class="col-sm-12">
                                ...
                                <label>订单编号</label>
                                <input type="text" id="shangpinOrderUuidNumberSearch" placeholder="请输入订单编号"
                                       aria-controls="tableId" class="form-control form-control-sm">

                                <label>订单类型</label>
                                <select name="shangpinOrderTypesSelectSearch" id="shangpinOrderTypesSelectSearch"
                                        aria-controls="tableId" class="form-control form-control-sm"></select>


                                <label>商品名称</label>
                                <input type="text" id="shangpinNameSearch" placeholder="请输入商品名称"
                                       aria-controls="tableId" class="form-control form-control-sm">

                                <label>商品类型</label>
                                <select name="shangpinTypesSelectSearch" id="shangpinTypesSelectSearch"
                                        aria-controls="tableId" class="form-control form-control-sm"></select>
                                ...
                                <span class="导入导出">
                                            导入<input name="file" id="shangpinOrderExcelFile" type="file"
                                                       class="btn btn-success"
                                                       style="width: 70px!important; line-height: 35px;">
                                            <button class="btn btn-success"
                                                    onclick="download('/upload/shangpinOrderMuBan.xls')">导入模板下载</button>
                                        </span>
                            ...
                            <table id="tableId" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th>商品名称<i id="shangpinNameIcon"></i></th>
                                    <th>商品照片<i id="shangpinPhotoIcon"></i></th>
                                    <th>商品类型<i id="shangpinTypesIcon"></i></th>
                                    <th>用户姓名<i id="yonghuNameIcon"></i></th>
                                    <th onclick="sort('shangpin_order_uuid_number')">订单编号<i
                                            id="shangpinOrderUuidNumberIcon" class="fa fa-sort"></i></th>
                                    <th onclick="sort('buy_number')">购买数量<i id="buyNumberIcon"
                                                                                class="fa fa-sort"></i></th>
                                    <th onclick="sort('shangpin_order_time')">预约时间<i id="shangpinOrderTimeIcon"
                                                                                         class="fa fa-sort"></i></th>
                                    <th onclick="sort('shangpin_order_true_price')">实付价格<i
                                            id="shangpinOrderTruePriceIcon" class="fa fa-sort"></i></th>
                                    <th onclick="sort('shangpin_order_types')">订单类型<i id="shangpinOrderTypesIcon"
                                                                                          class="fa fa-sort"></i></th>
...
<script>
    ...
    function setDataRow(item, number) {
        ...
        //预约时间
        var shangpinOrderTimeCell = document.createElement('td');
        shangpinOrderTimeCell.innerHTML = item.shangpinOrderTime;
        row.appendChild(shangpinOrderTimeCell);
        ...
        if (window.sessionStorage.getItem('role') == "用户" && item.shangpinOrderTypes == 101) {
            //退款
    ...
    function upload() {
        /*导入数据*/
        $('#shangpinOrderExcelFile').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: "json",
            done: function (e, data) {
                http("shangpinOrder/batchInsert?fileName=" + data.result.file, "get", {}, (res) => {
                    if (res.code == 0) {
    ...
    function wuyong(id) {
        ...
        if (mymessage == true) {
            httpJson("shangpinOrder/update", "POST", {
                id: id,
                shangpinOrderTypes: 2
            }, (res) => {
                if (res.code == 0) {
                    getDataList();
                    layui.layer.msg("操作成功", {time: 2000, icon: 6});
