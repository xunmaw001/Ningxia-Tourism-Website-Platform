                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 jingdainCollectionTypesDiv">
                                        <label>类型</label>
                                        <select style="width: 450px" id="jingdainCollectionTypesSelect" name="jingdainCollectionTypes" class="form-control"></select>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("jingdainCollection/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addjingdainCollection');
    ...
    // 查询列表
        ...
        function jingdainCollectionTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jingdain_collection_types", "GET", {}, (res) => {
                if(res.code == 0){
                    jingdainCollectionTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    // 初始化下拉框
    <!-- 初始化当前表的下拉框 -->
        function initializationJingdainCollectionTypesSelect(){
            var jingdainCollectionTypesSelect = document.getElementById('jingdainCollectionTypesSelect');
            if(jingdainCollectionTypesSelect != null && jingdainCollectionTypesOptions != null  && jingdainCollectionTypesOptions.length > 0 ){
                jingdainCollectionTypesSelect.options.length=0;
                for (var i = 0; i < jingdainCollectionTypesOptions.length; i++) {
                    jingdainCollectionTypesSelect.add(new Option(jingdainCollectionTypesOptions[i].indexName,jingdainCollectionTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(jingdainCollectionTypesSelect != null && jingdainCollectionTypesOptions != null  && jingdainCollectionTypesOptions.length > 0 ) {
            for (var i = 0; i < jingdainCollectionTypesOptions.length; i++) {
                if (jingdainCollectionTypesOptions[i].codeIndex == ruleForm.jingdainCollectionTypes) {//下拉框value对比,如果一致就赋值汉字
                        jingdainCollectionTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                jingdainId: "required",
                yonghuId: "required",
                jingdainCollectionTypes: "required",
            },
            messages: {
                jingdainId: "景点不能为空",
                yonghuId: "用户不能为空",
                jingdainCollectionTypes: "类型不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addjingdainCollection = window.sessionStorage.getItem("addjingdainCollection");
        if (addjingdainCollection != null && addjingdainCollection != "" && addjingdainCollection != "null") {
            //注册表单验证
            $(validform());
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            <!--  初始化当前表的下拉框  -->
            initializationJingdainCollectionTypesSelect();
            <!--  初始化级联表的下拉框  -->
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            if(sessionTable=="jingdainCollection"){//个人中心修改
            }
        }
