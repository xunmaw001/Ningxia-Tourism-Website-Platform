                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 jingdainCommentbackTextDiv">
                                        <label>评价内容</label>
                                        <textarea style="width: 450px"  id="jingdainCommentbackText" name="jingdainCommentbackText" class="form-control"placeholder="评价内容"></textarea>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("jingdainCommentback/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addjingdainCommentback');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                jingdainId: "required",
                yonghuId: "required",
                jingdainCommentbackText: "required",
                replyText: "required",
            },
            messages: {
                jingdainId: "景点不能为空",
                yonghuId: "用户不能为空",
                jingdainCommentbackText: "评价内容不能为空",
                replyText: "回复内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addjingdainCommentback = window.sessionStorage.getItem("addjingdainCommentback");
        if (addjingdainCommentback != null && addjingdainCommentback != "" && addjingdainCommentback != "null") {
            //注册表单验证
            $(validform());
            ...
            $('#submitBtn').text('修改');
            if(window.sessionStorage.getItem('accountTableName') =='jingdainCommentback'){
                updateId = window.sessionStorage.getItem('userId');//登录用户查看自己的信息
            }else{
                updateId = window.sessionStorage.getItem('updateId');//获取修改传过来的id
            }
            window.sessionStorage.removeItem('updateId');
            http("jingdainCommentback/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#jingdainCommentbackText").val(ruleForm.jingdainCommentbackText);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            $('#jingdainCommentbackText').attr('readonly', 'readonly');
            ...
            //设置留言或者评价不能修改
            $('#jingdainCommentbackText').attr('readonly', 'readonly');
            if(sessionTable=="jingdainCommentback"){//个人中心修改
            }
        }
